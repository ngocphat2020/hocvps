...
read -r -p "[$(date)] (Default password: @ngocphat):" db_pass
if [ -z "${db_pass}" ]; then
    db_pass="@ngocphat"
fi
_info "---------------------------"
...
_error_detect "ln -s /usr/share/zoneinfo/Asia/Ho_Chi_Minh /etc/localtime"
if check_sys rhel; then
...
_info "./mariadb_repo_setup.sh --mariadb-server-version=mariadb-10.6"
./mariadb_repo_setup.sh --mariadb-server-version=mariadb-10.6 >/dev/null 2>&1
